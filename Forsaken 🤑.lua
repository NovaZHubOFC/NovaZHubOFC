...
local function startSafeAutoGen()
	...
	autoGenThread = task-- ESP 100% RGB para todos os Generators
local RunService = game:GetService("RunService")

local function createESP(model)
    if not model:FindFirstChild("NovaESP") then
        local highlight = Instance.new("Highlight")
...
task.spawn(function()
    ...
    while true do
        ...
        for _, gen in ipairs(generators) do
            createESP(gen)
            local esp = gen:FindFirstChild("NovaESP")
            if esp then
                local color = RGB(t)
                esp.FillColor = color
...
--// ✔ BOTÃO QUE VOCÊ PEDIU (EXECUTA O SCRIPT EXTERNO)
KillersTab:CreateButton({
    Name = "Noli Void Rush 6️⃣7️⃣",
    Callback = function()
        pcall(function()
...
KillersTab:CreateButton({
    ...
    Callback = function()
        pcall(function()
            loadstring(game:HttpGet("https://raw.githubusercontent.com/NovaZHubOFC/NovaZHubOFC/main/Info2%F0%9F%9B%A1%EF%B8%8F.lua"))()
        end)
    end
})
